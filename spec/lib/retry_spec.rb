...
module RetrySpec
  RSpec.describe Ax1Utils::Retry do
    describe "#initialize" do
      context 'only count passed' do
        context 'not a positive integer' do
          it 'raises argument error' do
            [nil, 'string', -1, 0, 1.1].each do |invalid_count|
              expect { Ax1Utils::Retry.new(invalid_count) }.to raise_error(ArgumentError, "count must be a positive integer")
            end
          end
        end
        ...
        context 'positive integer' do
          it 'return Retry object' do
            [1, 10, 4].each do |valid_count|
              retrier  = Ax1Utils::Retry.new(valid_count)
      ...
      context 'count and rest_interval passed' do
        context 'not positive integers' do
          it 'return Retry object' do
            [nil, 'string', -1, 0, 1.1].each do |invalid_count|
              expect { Ax1Utils::Retry.new(invalid_count, 4) }.to raise_error(ArgumentError, "count must be a positive integer")
            end

            ['string', -1, 0, 1.1].each do |invalid_rest_interval|
              expect { Ax1Utils::Retry.new(1, invalid_rest_interval) }.to raise_error(ArgumentError, "rest_interval must be a positive integer")
            end

            [nil, 'string', -1, 0, 1.1].each do |invalid|
              expect { Ax1Utils::Retry.new(invalid, invalid) }.to raise_error(ArgumentError, "count must be a positive integer")
            end
          end
        end
        ...
        context 'positive integers' do
          it 'return Retry object' do
            retrier  = Ax1Utils::Retry.new(3, 10)
        ...
        context 'nil for rest_interval' do
          it 'return Retry object' do
            retrier  = Ax1Utils::Retry.new(3, nil)
            expect(retrier.instance_variable_get(:@count)).to eq(3)
    ...
    describe "#start (2 retries)" do
      let(:retrier) { Ax1Utils::Retry.new(2) }
